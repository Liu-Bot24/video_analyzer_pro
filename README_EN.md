...
| pi.model | The visual large model name to be called, supports hot-swapping at any time. |
...
| Parameter | Description |
| :--- | :--- |
| ideo.source_dir | Directory containing the raw videos to be processed (direct pasting of Windows absolute paths is supported). |
| ideo.dynamic_frames | **Fully Customizable Dynamic Stepping**. Format like [Duration Upper Limit(s), Frames to Extract], supports infinite steps. The program auto-calculates video duration and matches the optimal frames. |
| ideo.max_dimension | Upper limit for the long edge scaling of extracted images. Exceeding this pixel value will proportionally scale down the image. |
| ideo.extensions | Whitelist of video file extensions to be scanned (e.g., .mp4, .avi, etc.). |
| ideo.auto_rename | Switch: Whether to automatically physically rename files based on the "short title" generated by AI. |
| ideo.keep_original_name | **Renaming Strategy**: If 	rue, generates ShortTitle_OriginalFileName.mp4; if alse, only keeps ShortTitle.mp4 (built-in logic prevents overwriting via auto-increment suffixes). |
...
| Parameter | Description |
| :--- | :--- |
| system.concurrency | The number of concurrent threads to process. Default is 1, generally 2-4 is safer. |
